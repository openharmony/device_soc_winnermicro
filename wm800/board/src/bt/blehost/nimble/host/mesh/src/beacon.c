...
static struct bt_mesh_subnet *cache_check(u8_t data[21]) // 21:array length
{
    int i;
    ...
    for (i = 0; i < ARRAY_SIZE(bt_mesh.sub); i++) {
        ...
        if (!memcmp(sub->beacon_cache, data, 21)) { // 21:array length
            return sub;
        }
    }
                           ...
                           struct os_mbuf *buf)
{
    ...
    /* Network ID */
    net_buf_simple_add_mem(buf, keys->net_id, 8); // 8:byte alignment
    /* IV Index */
    net_buf_simple_add_be32(buf, bt_mesh.iv_index);
    net_buf_simple_add_mem(buf, sub->auth, 8); // 8:byte alignment
    BT_DBG("net_idx 0x%04x flags 0x%02x NetID %s", sub->net_idx,
           flags, bt_hex(keys->net_id, 8)); // 8:byte alignment
    BT_DBG("IV Index 0x%08x Auth %s", (unsigned) bt_mesh.iv_index,
           bt_hex(sub->auth, 8)); // 8:byte alignment
}
...
static int secure_beacon_send(void)
{
    ...
    for (i = 0; i < ARRAY_SIZE(bt_mesh.sub); i++) {
        ...
        if (time_diff < K_SECONDS(600) && // 600:time unit
...
static int unprovisioned_beacon_send(void)
{
    ...
    u8_t uri_hash[16] = { 0 }; // 16:array length
    struct os_mbuf *buf;
    u16_t oob_info;
    ...
    net_buf_add_mem(buf, prov->uuid, 16); // 16:byte alignment
    ...
    net_buf_add_mem(buf, uri_hash, 4); // 4:byte alignment
...
static void unprovisioned_beacon_recv(struct os_mbuf *buf)
{
    ...
    if (buf->om_len != 18 && buf->om_len != 22) { // 18:Analyzing conditions, 22:Analyzing conditions
        BT_ERR("Invalid unprovisioned beacon length (%u)", buf->om_len);
        return;
    }

    uuid = net_buf_simple_pull_mem(buf, 16); // 16:byte alignment
    oob_info = net_buf_simple_pull_be16(buf);

    if (buf->om_len == 4) { // 4:byte alignment
        uri_hash_val = net_buf_simple_pull_be32(buf);
        uri_hash = &uri_hash_val;
    }

    BT_DBG("uuid %s", bt_hex(uuid, 16)); // 16:byte alignment
    prov = bt_mesh_prov_get();
...
static void secure_beacon_recv(struct os_mbuf *buf)
{
    ...
    if (buf->om_len < 21) { // 21:Analyzing conditions
    ...
    net_id = net_buf_simple_pull_mem(buf, 8); // 8:byte alignment
    iv_index = net_buf_simple_pull_be32(buf);
    auth = buf->om_data;
    BT_DBG("flags 0x%02x id %s iv_index 0x%08x",
           flags, bt_hex(net_id, 8), (unsigned) iv_index); // 8:byte alignment
