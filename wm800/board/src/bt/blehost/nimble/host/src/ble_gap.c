...
void ble_gap_rx_peroidic_adv_sync_estab(const struct ble_hci_ev_le_subev_periodic_adv_sync_estab *ev)
{
    ...
    if (!ev->status) {
        ...
        memcpy(ble_gap_sync.psync->advertiser_addr.val, ev->peer_addr, 6); // 2:size
                                      ...
                                      *ev)
{
    ...
    if (ev->status) {
        ...
        memcpy(conn->psync->advertiser_addr.val, ev->peer_addr, 6); // 6:size
    ...
    memcpy(event.periodic_transfer.adv_addr.val, ev->peer_addr, 6); // 6:size
...
ble_gap_rx_conn_complete(struct ble_gap_conn_complete *evt, uint8_t instance)
{
...
#if MYNEWT_VAL(BLE_EXT_ADV)
        memcpy(conn->bhc_our_rnd_addr, ble_gap_slave[instance].rnd_addr, 6); // 6:size
#endif
        ble_gap_slave_reset_state(instance);
    }

    conn->bhc_peer_addr.type = evt->peer_addr_type;
    memcpy(conn->bhc_peer_addr.val, evt->peer_addr, 6); // 6:size
    conn->bhc_our_rpa_addr.type = BLE_ADDR_RANDOM;
    memcpy(conn->bhc_our_rpa_addr.val, evt->local_rpa, 6); // 6:size
    ...
    if (memcmp(BLE_ADDR_ANY->val, evt->peer_rpa, 6) == 0) { // 6:size
        ...
        memcpy(conn->bhc_peer_rpa_addr.val, evt->peer_rpa, 6); // 6:size
    }
...
ble_gap_ext_adv_set_addr_no_lock(uint8_t instance, const uint8_t *addr)
{
    ...
    memcpy(ble_gap_slave[instance].rnd_addr, addr, 6); // 6:size
    return 0;
}
