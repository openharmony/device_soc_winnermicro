...
static int ble_hs_flow_tx_num_comp_pkts(void)
{
    ...
    for (conn = ble_hs_conn_first();
        ...
        if (conn->bhc_completed_pkts > 0) {
            ...
            /* could combine this in single HCI command */
            cmd->handles = 1;
...
static os_error_t ble_hs_flow_acl_free(struct os_mempool_ext *mpe, void *data, void *arg)
{
    struct ble_hs_conn *conn;
