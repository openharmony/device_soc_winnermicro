 ...
 *                                  field.
 */
static int ble_att_svr_tx_rsp(uint16_t conn_handle, int hs_status, struct os_mbuf *om,
...
static struct ble_att_prep_entry *ble_att_svr_prep_find_prev(struct ble_att_svr_conn *basc,
                                  ...
                                  uint16_t *err_handle)
{
    ...
    *err_handle = 0; /* Silence unnecessary warning. */
    ...
    while (!SLIST_EMPTY(prep_list)) {
         ...
         * processing.
         */
        struct ble_att_svr_entry *attr = ble_att_svr_find_by_handle(attr_handle);
