...
static void *ble_eddystone_set_svc_data_base(uint8_t frame_type)
{
    ...
    ble_eddystone_svc_data[2] = frame_type; // 2:array element
                                   ...
                                   void *uid, int8_t measured_power)
{
    ...
    if (measured_power < -100 || measured_power > 20) { // -100:Analyzing conditions, 20:Analyzing conditions
        return BLE_HS_EINVAL;
    }
    ...
    rc = ble_eddystone_set_adv_data_gen(adv_fields, 19); // 19:svc_data_len
    if (rc != 0) {
        return rc;
    }
                                   ...
                                   int8_t measured_power)
{
    ...
    rc = ble_eddystone_set_adv_data_gen(adv_fields, url_len + 2); // 2:byte alignment
    if (rc != 0) {
        return rc;
    }
