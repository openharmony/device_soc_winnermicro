...
static void ble_l2cap_sig_dbg_assert_proc_not_inserted(struct ble_l2cap_sig_proc *proc)
{
...
static void ble_l2cap_sig_process_status(struct ble_l2cap_sig_proc *proc, int status)
{
    if (status == 0) {
...
static void ble_l2cap_sig_update_call_cb(struct ble_l2cap_sig_proc *proc, int status)
{
...
static void ble_l2cap_event_coc_connected(struct ble_l2cap_chan *chan, uint16_t status)
{
...
static int ble_l2cap_event_coc_accept(struct ble_l2cap_chan *chan, uint16_t peer_sdu_size)
{
    struct ble_l2cap_event event = { };
...
static void ble_l2cap_sig_coc_connect_cb(struct ble_l2cap_sig_proc *proc, int status)
{
    struct ble_hs_conn *conn;
    struct ble_l2cap_chan *chan;
    int i;
...
#if MYNEWT_VAL(BLE_L2CAP_ENHANCED_COC)
static void ble_l2cap_event_coc_reconfigured(uint16_t conn_handle, uint16_t status,
...
static int ble_l2cap_sig_credit_base_reconfig_req_rx(uint16_t conn_handle,
                                                     struct ble_l2cap_sig_hdr *hdr,
                                                     struct os_mbuf **om)
{
    ...
    if (!rsp) {
        /* Reuse request buffer for the response. For now in such a case
         * remote will timeout.
         */
...
static void ble_l2cap_sig_coc_reconfig_cb(struct ble_l2cap_sig_proc *proc, int status)
{
    int i;
    struct ble_l2cap_chan *chan[BLE_L2CAP_MAX_COC_CONN_REQ] = {0};
    struct ble_hs_conn *conn;
...
static int ble_l2cap_sig_credit_base_reconfig_rsp_rx(uint16_t conn_handle,
                                                     struct ble_l2cap_sig_hdr *hdr,
                                                     struct os_mbuf **om)
{
    struct ble_l2cap_sig_proc *proc;
                                                ...
                                                struct os_mbuf **om)
{
    ...
    if (num_of_scids > 5) { // 5:Analyzing conditions
        rsp->result = htole16(BLE_L2CAP_COC_ERR_INVALID_PARAMETERS);
        goto failed;
    }
...
static int ble_l2cap_sig_coc_req_rx(uint16_t conn_handle, struct ble_l2cap_sig_hdr *hdr,
                                    struct os_mbuf **om)
{
    int rc;
...
static int ble_l2cap_sig_coc_rsp_rx(uint16_t conn_handle, struct ble_l2cap_sig_hdr *hdr,
                                    struct os_mbuf **om)
{
    struct ble_l2cap_sig_proc *proc;
    struct ble_l2cap_sig_le_con_rsp *rsp;
...
static int ble_l2cap_sig_disc_req_rx(uint16_t conn_handle, struct ble_l2cap_sig_hdr *hdr,
                                     struct os_mbuf **om)
{
...
static void ble_l2cap_sig_coc_disconnect_cb(struct ble_l2cap_sig_proc *proc, int status)
{
    struct ble_l2cap_chan *chan;
    struct ble_l2cap_event event;
    struct ble_hs_conn *conn;
...
static int ble_l2cap_sig_le_credits_rx(uint16_t conn_handle, struct ble_l2cap_sig_hdr *hdr,
                                       struct os_mbuf **om)
{
...
int ble_l2cap_sig_le_credits(uint16_t conn_handle, uint16_t scid, uint16_t credits)
{
 ...
 *                                  occurs.
 */
static int32_t ble_l2cap_sig_extract_expired(struct ble_l2cap_sig_proc_list *dst_list)
{
    struct ble_l2cap_sig_proc *proc;
    struct ble_l2cap_sig_proc *prev;
