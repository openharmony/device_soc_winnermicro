...
extern int tls_ble_deregister_report_result(tls_ble_dm_evt_t rpt_evt,
    tls_ble_dm_callback_t rpt_callback);
...
extern int tls_ble_gap_set_adv_param(uint32_t min, uint32_t max, uint8_t chn_map,
    uint8_t filter_policy, uint8_t *dir_mac);
extern int tls_ble_gap_set_scan_param(uint32_t intv, uint32_t window,
    uint8_t filter_policy, bool limited, bool passive, bool filter_duplicate);
...
#define CMD_RSP_BUF_SIZE    600  // 256
    ...
    switch (hostif_cmd_type) {
        ...
        case HOSTIF_UART1_RI_CMD:
            ...
            tls_hostif_fill_hdr(hif, (struct tls_hostif_hdr *)cmdrsp_buf,
                    PACKET_TYPE_RI_CMD, cmdrsp_size, 0, 0, 0);
...
#if TLS_CONFIG_RMMS
    if (hostif_cmd_type == HOSTIF_RMMS_AT_CMD) {
        memcpy(buf + SIX, cmdrsp_buf, cmdrsp_size + 1);
        memcpy(cmdrsp_buf, buf, SIX + cmdrsp_size + 1);
        cmdrsp_size = cmdrsp_size + SIX;
        tls_mem_free(buf);
    }
#endif
    ...
    enum tls_cmd_mode cmd_mode)
{
    ...
    switch (cmd_mode) {
        ...
        case CMD_MODE_UART1_RICMD:
            buflen = ONE_THOUSAND_FOUR_HUNRED_AND_FIFTY;
            buf = (char *)tls_mem_alloc(buflen);
            if (!buf) {
                return 0;
            }
                ...
                *p++ = (u8)scan_res->count;
                ...
                for (i = 0; i < scan_res->count; i++) {
                    ...
                    *p++ = (signed char)bss_info->rssi;
                    buflen += (TEN + 1 + bss_info->ssid_len);
                    remain_len = remain_len - (TEN + 1 + bss_info->ssid_len);
                    bss_info++;
                }
            }

            err = tls_hostif_send_event(buf, buflen, HOSTIF_EVENT_SCAN_RES);
            break;
    ...
    char *buf, u32 buflen)
{
    ...
    if (skt_info->socket) {
        ...
        err = tls_socket_udp_sendto(skt_info->local_port, 
                                   (u8 *)(&skt_info->remote_ip), skt_info->remote_port, buf, buflen);
    }
#else
    if (FD_ISSET(skt_info->socket, &fdatsockets)) {
        ...
        u32 skt_len;
        int n;
        skt_len = sizeof(struct tls_skt_status_ext_t) * FIVE + sizeof(u32);
        skt_status = (struct tls_skt_status_t *)tls_mem_alloc(skt_len);
        if (skt_status == NULL) {
            return ERR_MEM;
        }
        ...
        if (ext->protocol == SOCKET_PROTO_UDP) {
            err = sendto(skt_info->socket, buf, buflen, 0,
                         &sock_cmdp_addrs[skt_info->socket - LWIP_SOCKET_OFFSET],
    ...
    enum tls_cmd_mode cmd_mode)
{
    ...
    MEMCPY(skt_desc.host_name, skt->host_name, THIRTY + TWO);
    skt_desc.host_len = skt->host_len;
    MEMCPY(ip_2_ip4(&skt_desc.ip_addr), skt->ip_addr, FOUR);
    ...
    if ((skt->localport && (SOCKET_PROTO_UDP == skt->proto || skt->client)) ||
        (SOCKET_PROTO_TCP == skt->proto && !skt->client)) {
    ...
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    ...
    memcpy(socket.ip_addr, cmd->skct.ip_addr, FOUR);
...
sem_acquire:
            ...
            err = tls_os_sem_acquire(hif->uart_atcmd_sem, (TWENTY + FIVE) * HZ - offset);
            if (err) {
                return -CMD_ERR_SKT_CONN;
            } else {
                if (!(hif->uart_atcmd_bits & (1 << UART_ATCMD_BIT_SKCT))) {
                    offset = tls_os_get_time() - time;
                    if (offset < (TWENTY + FIVE) * HZ) {
                        goto sem_acquire;
                    }
                }
...
int oneshot_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    int ret = 0;
    if (set_opt) {
#if TLS_CONFIG_BT
        if (cmd->oneshot.status > FOUR) {
#else
        if (cmd->oneshot.status > THREE) {
#endif
            return -CMD_ERR_INV_PARAMS;
        }
...
s32 tls_uart_bps_set(u8 portNum, u32 bdrate)
{
    ...
    val = (sysclk.apbclk * ONE_MILLION / ((TEN + SIX) * bdrate) - 1) |
          (((sysclk.apbclk * ONE_MILLION % (bdrate * (TEN + SIX))) * (TEN + SIX) / (bdrate * (TEN + SIX))) << (TEN + SIX));
            ...
            if (hif->uart_atcmd_bits & (1 << UART_ATCMD_BIT_ACTIVE_BT_DM_EXT)) {
                if (msg->adapter_prop.status == TLS_BT_STATUS_SUCCESS) {
                    if (msg->adapter_prop.num_properties == 1) {
                        ...
                        memcpy(buf + len, msg->adapter_prop.properties->val,
                               msg->adapter_prop.properties->len);
...
int ble_adv_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    return;
}

int ble_demo_adv_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    return;
}

int ble_adv_data_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    return;
}

int ble_scan_rsp_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    return;
}

int ble_adv_param_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    return;
}

int ble_scan_param_proc(u8 set_opt, u8 update_flash,
    union HOSTIF_CMD_PARAMS_UNION *cmd, union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    return;
}

int ble_scan_filter_proc(u8 set_opt, u8 update_flash,
    union HOSTIF_CMD_PARAMS_UNION *cmd, union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    return 0;
}
...
static u8 get_valid_adv_length_and_name(uint8_t *ptr, uint8_t *pname)
{
    ...
    while (seg_len != 0) {
        if (ptr[ret + 1] == 0x09 || ptr[ret + 1] == 0x08) {
            min_len = MIN((SIXTY + FOUR), seg_len - 1);
            memcpy(pname, ptr + ret + TWO, min_len);
            pname[min_len] = 0;
            asm("nop");
            asm("nop");
            asm("nop");
...
int ble_read_rssi_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    tls_bt_status_t ret;
#if (WM_BLE_INCLUDED == CFG_ON)
    ...
    bt_wait_rsp_timeout(cmd->bt.cmd_mode, cmd, hif, FIVE);
#else
    printf("TODO TODO\r\n");
#endif  
 end_tag:
...
int ble_demo_send_indicate(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    return;
}

int ble_demo_scan_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    return;
}

int ble_scan_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    return;
}

int ble_demo_server_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    return;
}

int ble_demo_client_multi_conn_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    return;
}

int ble_demo_client_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    return;
}

int ble_scan_chnl_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    tls_bt_status_t ret;
...
int ble_uart_mode_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    return;
}
...
#define BLE_EVT_BUF_LEN 1248
    ...
    switch (event) {
        ...
        case WM_BLE_SE_CREATE_EVT:
            len = sprintf(buf, "+OK=%hhu,%hhu,%hhu\r\n", msg->ser_create.status,
                          msg->ser_create.server_if, msg->ser_create.service_id);
            ret = tls_hostif_process_cmdrsp(hostif_type, buf, len);
            break;
        case WM_BLE_SE_ADD_CHAR_EVT:
            len = sprintf(buf, "+OK=%hhu,%hhu,%hhu,%hhu\r\n", msg->ser_add_char.status,
                          msg->ser_add_char.server_if, msg->ser_add_char.service_id, msg->ser_add_char.attr_id);
            ret = tls_hostif_process_cmdrsp(hostif_type, buf, len);
            break;
        case WM_BLE_SE_ADD_CHAR_DESCR_EVT:
            len = sprintf(buf, "+OK=%hhu,%hhu,%hhu,%hhu\r\n", msg->ser_add_char_descr.status,
                          msg->ser_add_char_descr.server_if, msg->ser_add_char_descr.service_id,
                          msg->ser_add_char_descr.attr_id);
            ret = tls_hostif_process_cmdrsp(hostif_type, buf, len);
            break;
        case WM_BLE_SE_START_EVT:
            len = sprintf(buf, "+OK=%hhu,%hhu,%hhu\r\n", msg->ser_start_srvc.status,
                          msg->ser_start_srvc.server_if, msg->ser_start_srvc.service_id);
            ret = tls_hostif_process_cmdrsp(hostif_type, buf, len);
            break;
        case WM_BLE_SE_STOP_EVT:
            len = sprintf(buf, "+OK=%hhu,%hhu,%hhu\r\n", msg->ser_stop_srvc.status,
                          msg->ser_stop_srvc.server_if, msg->ser_stop_srvc.service_id);
            ret = tls_hostif_process_cmdrsp(hostif_type, buf, len);
        ...
        case WM_BLE_SE_DELETE_EVT:
            len = sprintf(buf, "+OK=%hhu,%hhu,%hhu\r\n", msg->ser_delete_srvc.status,
                          msg->ser_delete_srvc.server_if, msg->ser_delete_srvc.service_id);
            ret = tls_hostif_process_cmdrsp(hostif_type, buf, len);
            if (hif->uart_atcmd_bits & (1 << UART_ATCMD_BIT_ACTIVE_BT)) {
                passive_response =0;
                hif->uart_atcmd_bits &= ~(1 << UART_ATCMD_BIT_ACTIVE_BT);
        ...
        case WM_BLE_SE_CONNECT_EVT:
            len = sprintf(buf, "+OK=0,%hhu,%hu,%hhu,%02X%02X%02X%02X%02X%02X\r\n",
                msg->ser_connect.server_if, msg->ser_connect.conn_id,
                msg->ser_connect.connected, msg->ser_connect.addr[0],
                msg->ser_connect.addr[1], msg->ser_connect.addr[TWO], msg->ser_connect.addr[THREE],
                msg->ser_connect.addr[FOUR], msg->ser_connect.addr[FIVE]);
            ret = tls_hostif_process_cmdrsp(hostif_type, buf, len);
            if (hif->uart_atcmd_bits & (1 << UART_ATCMD_BIT_ACTIVE_BT)) {
                passive_response = 0;
                hif->uart_atcmd_bits &= ~(1 << UART_ATCMD_BIT_ACTIVE_BT);
        ...
        case WM_BLE_SE_WRITE_EVT:
            len = sprintf(buf, "+OK=0,%hu,%hhu,%hhu,%hhu,", msg->ser_write.conn_id,msg->ser_write.handle,
                          msg->ser_write.trans_id, msg->ser_write.len);

            tmp_len = MIN(TWO_HUNDRED_AND_FIFTY_SIX, msg->ser_write.len);
        ...
        case WM_BLE_CL_OPEN_EVT:
            len = sprintf(buf, "+OK=%hhu,%hhu,%hu\r\n", msg->cli_open.status,
                          msg->cli_open.client_if, msg->cli_open.conn_id);
            ret = tls_hostif_process_cmdrsp(hostif_type, buf, len);
            if (hif->uart_atcmd_bits & (1 << UART_ATCMD_BIT_ACTIVE_BT)) {
                passive_response = 0;
                hif->uart_atcmd_bits &= ~(1 << UART_ATCMD_BIT_ACTIVE_BT);
        ...
        case WM_BLE_CL_CLOSE_EVT:
            ...
            */
            len = sprintf(buf, "+OK=%hhu,%hu,%hhu,%hhu\r\n",0, msg->cli_close.client_if,
                          msg->cli_close.conn_id, msg->cli_close.reason);
            ret = tls_hostif_process_cmdrsp(hostif_type, buf, len);
            if (hif->uart_atcmd_bits & (1 << UART_ATCMD_BIT_ACTIVE_BT)) {
                passive_response = 0;
                hif->uart_atcmd_bits &= ~(1 << UART_ATCMD_BIT_ACTIVE_BT);
        ...
        case WM_BLE_CL_REPORT_DB_EVT:
            ...
            for (index = 0; index < msg->cli_db.count; index++) {
                if (msg->cli_db.db) {
                    memcpy(&uuid, msg->cli_db.db->uuid.uu + (TEN + TWO), TWO);
                    if (msg->cli_db.db->type == 0) {
                        len += sprintf(buf + len, "0x%04x,T=0x%02x,HDL=%hhu,PROP=0x%02x \r\n", uuid,
                                       msg->cli_db.db->type, msg->cli_db.db->attribute_handle, msg->cli_db.db->properties);
                    } else {
                        len += sprintf(buf + len, "\t0x%04x,T=0x%02x,HDL=%hhu,PROP=0x%02x \r\n", uuid,
                                       msg->cli_db.db->type, msg->cli_db.db->attribute_handle, msg->cli_db.db->properties);
                    }
        ...
        case WM_BLE_CL_REG_NOTIFY_EVT:
            len = sprintf(buf, "+OK=%hhu,%hu,%hhu\r\n", msg->cli_reg_notify.status,
                          msg->cli_reg_notify.conn_id, msg->cli_reg_notify.handle);
            ret = tls_hostif_process_cmdrsp(hostif_type, buf, len);
            break;
        ...
        case WM_BLE_CL_READ_DESCR_EVT:
            ...
            tmp_len = MIN(TWO_HUNDRED_AND_FIFTY_SIX, msg->cli_read.len);
            for (index = 0; index < tmp_len; index++) {
        ...
        case WM_BLE_CL_WRITE_DESCR_EVT:
            len = sprintf(buf, "+OK=%hhu,%hu,%hhu\r\n", msg->cli_write.status,
                          msg->cli_write.conn_id, msg->cli_write.handle);
            ret = tls_hostif_process_cmdrsp(hostif_type, buf, len);
            break;
        case WM_BLE_CL_DEREG_NOTIFY_EVT:
            len = sprintf(buf, "+OK=%hhu,%hhu,%hu\r\n", msg->cli_reg_notify.status,
                          msg->cli_reg_notify.handle, msg->cli_reg_notify.conn_id);
            ret = tls_hostif_process_cmdrsp(hostif_type, buf, len);
            break;
        ...
        case WM_BLE_CL_CFG_MTU_EVT:
            len = sprintf(buf, "+OK=%hhu,%hu,%hu\r\n", msg->cli_cfg_mtu.status, 
                          msg->cli_cfg_mtu.conn_id, msg->cli_cfg_mtu.mtu);
            ret = tls_hostif_process_cmdrsp(hostif_type, buf, len);
            if (hif->uart_atcmd_bits & (1 << UART_ATCMD_BIT_ACTIVE_BT)) {
                passive_response = 0;
                hif->uart_atcmd_bits &= ~(1 << UART_ATCMD_BIT_ACTIVE_BT);
...
int ble_create_server_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    tls_bt_status_t ret;
    struct tls_hostif *hif = tls_get_hostif();
...
int ble_add_service_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    tls_bt_status_t ret;
    struct tls_hostif *hif = tls_get_hostif();
    ...
    ret = tls_ble_server_add_service(cmd->blesc.server_if, cmd->blesc.inst_id, 1,
                                     app_uuid16_to_uuid128(cmd->blesc.uuid), cmd->blesc.handles);
    if (ret != TLS_BT_STATUS_SUCCESS) {
        goto end_tag;
    }
...
int ble_add_characteristic_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    tls_bt_status_t ret;
    struct tls_hostif *hif = tls_get_hostif();
    ...
    ret = tls_ble_server_add_characteristic(cmd->blech.server_if, cmd->blech.service_handle,
    app_uuid16_to_uuid128(cmd->blech.uuid), cmd->blech.prop, cmd->blech.perm);
...
int ble_add_descript_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    tls_bt_status_t ret;
    struct tls_hostif *hif = tls_get_hostif();
    ...
    ret = tls_ble_server_add_descriptor(cmd->blech.server_if, cmd->blech.service_handle,
    app_uuid16_to_uuid128(cmd->blech.uuid), cmd->blech.perm);
    if (ret != TLS_BT_STATUS_SUCCESS) {
        goto end_tag;
    }
...
int ble_start_service_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    tls_bt_status_t ret;
    struct tls_hostif *hif = tls_get_hostif();
...
int ble_stop_service_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    tls_bt_status_t ret;
    struct tls_hostif *hif = tls_get_hostif();
    ...
    ret = bt_wait_rsp_timeout(cmd->blestt.cmd_mode, cmd, hif, THREE);

end_tag:
    return (ret == TLS_BT_STATUS_SUCCESS) ? 0 : -CMD_ERR_OPS;
}

int ble_del_service_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    tls_bt_status_t ret;
    struct tls_hostif *hif = tls_get_hostif();
    ...
    ret = bt_wait_rsp_timeout(cmd->blestt.cmd_mode, cmd, hif, FIVE);

end_tag:
    return (ret == TLS_BT_STATUS_SUCCESS) ? 0 : -CMD_ERR_OPS;
}

int ble_destory_server_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    tls_bt_status_t ret;
    struct tls_hostif *hif = tls_get_hostif();
    ...
    ret = bt_wait_rsp_timeout(cmd->bt.cmd_mode, cmd, hif, FIVE);

end_tag:
    return (ret == TLS_BT_STATUS_SUCCESS) ? 0 : -CMD_ERR_OPS;
}

int ble_server_connect_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    tls_bt_status_t ret;
    tls_bt_addr_t addr;
    ...
    memcpy(addr.address, cmd->bleconn.addr, SIX); asm("nop"); asm("nop"); asm("nop");
    ...
    */
    hif->uart_atcmd_bits |= (1 << UART_ATCMD_BIT_ACTIVE_BT);
    ret = bt_wait_rsp_timeout(cmd->bleconn.cmd_mode, cmd, hif, FIVE);

end_tag:
    return (ret == TLS_BT_STATUS_SUCCESS) ? 0 : -CMD_ERR_OPS;
}

int ble_server_disconnect_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    tls_bt_status_t ret;
    tls_bt_addr_t addr;
    ...
    hif->uart_atcmd_bits |= (1 << UART_ATCMD_BIT_ACTIVE_BT);
    ret = bt_wait_rsp_timeout(cmd->bleconn.cmd_mode, cmd, hif, EIGHT);

end_tag:
    return (ret == TLS_BT_STATUS_SUCCESS) ? 0 : -CMD_ERR_OPS;
}

int ble_server_send_indication_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    tls_bt_status_t ret;
    struct tls_hostif *hif = tls_get_hostif();
    ...
    ret = tls_ble_server_send_indication(cmd->blesndind.server_if, cmd->blesndind.attr_handle, cmd->blesndind.conn_id,
                                         strlen((const char *)cmd->blesndind.value), 1,
                                         (char *)cmd->blesndind.value);
    if (ret != TLS_BT_STATUS_SUCCESS) {
        goto end_tag;
    }

    ret = bt_wait_rsp_timeout(cmd->blesndind.cmd_mode, cmd, hif, FIVE);

end_tag:
    return (ret == TLS_BT_STATUS_SUCCESS) ? 0 : -CMD_ERR_OPS;
}

int ble_server_send_resp_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    tls_bt_status_t ret;
    struct tls_hostif *hif = tls_get_hostif();
    ...
    ret = tls_ble_server_send_response(cmd->blesndrsp.conn_id, cmd->blesndrsp.trans_id, 0, 0,
                                       cmd->blesndrsp.attr_handle,
                                       cmd->blesndrsp.auth_req, cmd->blesndrsp.value,
                                       strlen((const char *)cmd->blesndrsp.value));
    if (ret != TLS_BT_STATUS_SUCCESS) {
        goto end_tag;
    }

    ret = bt_wait_rsp_timeout(cmd->blesndrsp.cmd_mode, cmd, hif, FIVE);

end_tag:
    return (ret == TLS_BT_STATUS_SUCCESS) ? 0 : -CMD_ERR_OPS;
}

int ble_client_create_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    tls_bt_status_t ret;
    struct tls_hostif *hif = tls_get_hostif();
...
int ble_client_connect_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    tls_bt_status_t ret;
    tls_bt_addr_t addr;
    ...
    memcpy(&addr.address, cmd->bleconn.addr, SIX);
    ...
    hif->uart_atcmd_bits |= (1 << UART_ATCMD_BIT_ACTIVE_BT);
    ret = bt_wait_rsp_timeout(cmd->bleconn.cmd_mode, cmd, hif, THIRTY + 1);

end_tag:
    return (ret == TLS_BT_STATUS_SUCCESS) ? 0 : -CMD_ERR_OPS;
}

int ble_client_scan_service_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    tls_bt_status_t ret;
    struct tls_hostif *hif = tls_get_hostif();
...
int ble_client_get_gatt_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    tls_bt_status_t ret;
    struct tls_hostif *hif = tls_get_hostif();
...
int ble_client_reg_notify_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    tls_bt_status_t ret;
    struct tls_hostif *hif = tls_get_hostif();
    ...
    ret = tls_ble_client_register_for_notification(cmd->blenty.client_if, (const tls_bt_addr_t *)cmd->blenty.addr,
                                                   cmd->blenty.attr_handle, cmd->blenty.conn_id);
    if (ret != TLS_BT_STATUS_SUCCESS) {
        goto end_tag;
    }

    ret = bt_wait_rsp_timeout(cmd->blenty.cmd_mode, cmd, hif, FIVE);

end_tag:
    return (ret == TLS_BT_STATUS_SUCCESS) ? 0 : -CMD_ERR_OPS;
}

int ble_client_dereg_notify_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    tls_bt_status_t ret;
    struct tls_hostif *hif = tls_get_hostif();
    ...
    ret = tls_ble_client_deregister_for_notification(cmd->blenty.client_if, (const tls_bt_addr_t *)cmd->blenty.addr,
                                                     cmd->blenty.attr_handle, cmd->blenty.conn_id);
    if (ret != TLS_BT_STATUS_SUCCESS) {
        goto end_tag;
    }
...
int ble_client_access_characteristic_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    tls_bt_status_t ret;
    struct tls_hostif *hif = tls_get_hostif();
    ...
    if (cmd->bleacc.mode == 0) {
        ret = tls_ble_client_write_characteristic(cmd->bleacc.conn_id, cmd->bleacc.handle, 0, cmd->bleacc.data_len,
                                                  cmd->bleacc.auth_req, (char *)cmd->bleacc.data);
    } else {
        ret = tls_ble_client_read_characteristic(cmd->bleacc.conn_id, cmd->bleacc.handle,
                                                 cmd->bleacc.auth_req);
    }
    ...
    ret = bt_wait_rsp_timeout(cmd->bleacc.cmd_mode, cmd, hif, FIVE);

end_tag:
    return (ret == TLS_BT_STATUS_SUCCESS) ? 0 : -CMD_ERR_OPS;
}

int ble_client_disconnect_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    tls_bt_status_t ret;
    struct tls_hostif *hif = tls_get_hostif();
    ...
    ret = tls_ble_client_disconnect(cmd->bleconn.server_if, (const tls_bt_addr_t *)cmd->bleconn.addr,
                                    cmd->bleconn.conn_id);
    if (ret != TLS_BT_STATUS_SUCCESS) {
        goto end_tag;
    }
    hif->uart_atcmd_bits |= (1 << UART_ATCMD_BIT_ACTIVE_BT);
    ret = bt_wait_rsp_timeout(cmd->bleconn.cmd_mode, cmd, hif, (TEN + 1));

end_tag:
    return (ret == TLS_BT_STATUS_SUCCESS) ? 0 : -CMD_ERR_OPS;
}

int ble_client_destory_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    tls_bt_status_t ret;
    struct tls_hostif *hif = tls_get_hostif();
...
int ble_client_cfg_mtu_proc(u8 set_opt, u8 update_flash, union HOSTIF_CMD_PARAMS_UNION *cmd,
    union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp)
{
    tls_bt_status_t ret;
    struct tls_hostif *hif = tls_get_hostif();
    ...
    hif->uart_atcmd_bits |= (1 << UART_ATCMD_BIT_ACTIVE_BT);
    ret = bt_wait_rsp_timeout(cmd->blecmtu.cmd_mode, cmd, hif, FIVE);
...
static struct tls_cmd_t  at_ri_cmd_tbl[] = {
    ...
    { "BLEAPRM", HOSTIF_CMD_NOP, ATCMD_OP_NULL | ATCMD_OP_EQ | ATCMD_OP_EP | ATCMD_OP_QU | RICMD_OP_SET
    | RICMD_OP_SET, FIVE, 0, ble_adv_param_proc},
    { "BLESCPRM", HOSTIF_CMD_NOP, ATCMD_OP_EQ | ATCMD_OP_EP | RICMD_OP_SET, THREE, 0, ble_scan_param_proc},
    ...
    { "BLEADDSC", HOSTIF_CMD_NOP, ATCMD_OP_EQ | ATCMD_OP_EP | RICMD_OP_SET, FOUR, 0, ble_add_service_proc},
    { "BLEADDCH", HOSTIF_CMD_NOP, ATCMD_OP_EQ | ATCMD_OP_EP | RICMD_OP_SET, FIVE, 0, ble_add_characteristic_proc},
    { "BLEADESC", HOSTIF_CMD_NOP, ATCMD_OP_EQ | ATCMD_OP_EP | RICMD_OP_SET, FOUR, 0, ble_add_descript_proc},
    { "BLESTTSC", HOSTIF_CMD_NOP, ATCMD_OP_EQ | ATCMD_OP_EP | RICMD_OP_SET, THREE, 0, ble_start_service_proc},
    ...
    { "BLESVDIS", HOSTIF_CMD_NOP, ATCMD_OP_EQ | ATCMD_OP_EP | RICMD_OP_SET, THREE, 0, ble_server_disconnect_proc},
    { "BLESIND", HOSTIF_CMD_NOP, ATCMD_OP_EQ | ATCMD_OP_EP | RICMD_OP_SET, FOUR, 0, ble_server_send_indication_proc},
    { "BLESRSP", HOSTIF_CMD_NOP, ATCMD_OP_EQ | ATCMD_OP_EP | RICMD_OP_SET, FIVE, 0, ble_server_send_resp_proc},
    ...
    { "BLECRNTY", HOSTIF_CMD_NOP, ATCMD_OP_EQ | ATCMD_OP_EP | RICMD_OP_SET, FOUR, 0, ble_client_reg_notify_proc},
    { "BLECDNTY", HOSTIF_CMD_NOP, ATCMD_OP_EQ | ATCMD_OP_EP | RICMD_OP_SET, FOUR, 0, ble_client_dereg_notify_proc},
    { "BLECACH", HOSTIF_CMD_NOP, ATCMD_OP_EQ | ATCMD_OP_EP | RICMD_OP_SET, FOUR, 0, ble_client_access_characteristic_proc},
    { "BLECDIS", HOSTIF_CMD_NOP, ATCMD_OP_EQ | ATCMD_OP_EP | RICMD_OP_SET, THREE, 0, ble_client_disconnect_proc},
...
int at_parse_func(char *at_name, struct tls_atcmd_token_t *tok, union HOSTIF_CMD_PARAMS_UNION *cmd)
{
    ...
    ||(strcmp("APKEY", at_name) == 0)
        ...
        if (tok->arg_found >= 1) {
            ...
            if (((cmd->bssid.enable == 0) && (tok->arg_found == TWO)) ||
                ((cmd->bssid.enable == 1) && (tok->arg_found == 1)))
                    return -CMD_ERR_INV_PARAMS;
            if (tok->arg_found == TWO) {
    ...
    || (strcmp("BTEN", at_name) == 0) || (strcmp("BLEUM", at_name) == 0)
        ...
        u32 params;
        u8 *tmpmac = NULL;
        if ((tok->arg_found < FIVE) || (tok->arg_found > EIGHT)) {
            return -CMD_ERR_INV_PARAMS;
        }
        ...
        ret = string_to_uint(tok->arg[FOUR], &params);
        if (ret) {
            return -CMD_ERR_INV_PARAMS;
        }
        ...
        switch (tok->arg_found) {
            case EIGHT:
                if (atcmd_filter_quotation(&tmpmac, (u8 *)tok->arg[SEVEN])) {
                    return -CMD_ERR_INV_PARAMS;
                }
            ...
            case SEVEN:
                ret = string_to_uint(tok->arg[SIX], &params);
                if (ret) {
                    return -CMD_ERR_INV_PARAMS;
                }
                cmd->bleprm.peer_addr_type = (u8)params;
            case SIX:
                ret = string_to_uint(tok->arg[FIVE], &params);
                if (ret) {
                    return -CMD_ERR_INV_PARAMS;
                }
        ...
        ret = hexstr_to_unit(tok->arg[THREE], &params);
        if (ret) {
            return -CMD_ERR_INV_PARAMS;
        }
        cmd->blech.prop = params;
        ret = hexstr_to_unit(tok->arg[FOUR], &params);
        if (ret) {
            return -CMD_ERR_INV_PARAMS;
        }
        ...
        strncpy((char *)cmd->blesndind.value, tok->arg[THREE], sizeof(cmd->blesndind.value));
        ...
        strncpy((char *)cmd->blesndrsp.value, tok->arg[FOUR], sizeof(cmd->blesndind.value));
        ...
        if (cmd->bleacc.mode == 0) {
            ret = atcmd_filter_quotation(&tmpdata, (u8 *)tok->arg[FOUR]);
            if (ret) {
                return -CMD_ERR_INV_PARAMS;
            }
    ...
    ||(strcmp("APNIP", at_name) == 0)
        ...
        u32 params;
        u8 *tmpbuf=NULL;
        if (tok->arg_found != 0 && tok->arg_found != 1 && tok->arg_found != FIVE) {
            return -CMD_ERR_INV_PARAMS;
        }
        if (tok->arg_found > 0) {
            do {
                ...
                if ((tok->arg_found == 1 && cmd->nip.type != 0) ||
                    (tok->arg_found == FIVE && cmd->nip.type != 1)) {
                    err = 1;
                    break;
                }
                if (tok->arg_found == 1)
                ...
                MEMCPY(cmd->nip.ip, (u8 *)&params, FOUR);
                /* netmask */
                ...
                MEMCPY(cmd->nip.nm, (u8 *)&params, FOUR);
                /* gateway */
                ret = atcmd_filter_quotation(&tmpbuf, (u8 *)tok->arg[THREE]);
                if (ret) {
                    err = 1;
                    break;
                }
                ...
                MEMCPY(cmd->nip.gw, (u8 *)&params, FOUR);
                /* dns */
                ret = atcmd_filter_quotation(&tmpbuf, (u8 *)tok->arg[FOUR]);
                if (ret) {
                    err = 1;
                    break;
                }
                ...
                MEMCPY(cmd->nip.dns, (u8 *)&params, FOUR);
        ...
        u8 *tmp;
        struct tls_cmd_socket_t socket;
        if (tok->arg_found != 0 && tok->arg_found != FOUR) {
            return -CMD_ERR_INV_PARAMS;
        }
        if (tok->arg_found == FOUR) {
            do {
            ...
            if (err) {
                ...
                memcpy(cmd->atrm.ip_addr, socket.ip_addr, FOUR);
        ...
        int ret;
        u32 params;
        if (tok->arg_found != 0 && tok->arg_found != FOUR && tok->arg_found != FIVE) {
            return -CMD_ERR_INV_PARAMS;
        }
        if (tok->arg_found >= FOUR) {
            do {
                ...
                MEMCPY(cmd->uart.baud_rate, (u8 *)&params, THREE);
                /* char length */
                ret = string_to_uint(tok->arg[1], &params);
                if (ret) {
                    err = 1;
        ...
        do {
            ...
            socket.client = (u8)params ? 0 : 1;
            host_len = tok->arg[THREE] - tok->arg[TWO] - 1;
            if (host_len > (THIRTY + TWO)) {
                err = 1;
                break;
            }
            /* check ip or timeout  */
            if (socket.client) {
                ...
                if (!ret) {
                    ...
                    HostEntry = gethostbyname((char *)ipstr);
                    if (HostEntry) {
                        MEMCPY(socket.ip_addr, HostEntry->h_addr_list[0], FOUR);
                    } else {
                        err = 1;
                        break;
                    }
            ...
            if ((tok->arg_found == FOUR) && (params == 0)) {
                err = 1;
                break;
            }
            socket.port = params;
        ...
        if (err) {
            ...
            memcpy(cmd->skct.ip_addr, socket.ip_addr, FOUR);
        ...
        ret = hexstr_to_unit(tok->arg[THREE], (u32 *)&cmd->lptstr.txgain);
        if (ret) {
            return -CMD_ERR_INV_PARAMS;
        }
        ret = hexstr_to_unit(tok->arg[FOUR], (u32 *)&cmd->lptstr.datarate);
        if (ret) {
            return -CMD_ERR_INV_PARAMS;
        }
        switch (tok->arg_found) {
            case EIGHT:
                ret = hexstr_to_unit(tok->arg[SEVEN], (u32 *)&cmd->lptstr.gimode);
                if (ret) {
                    return -CMD_ERR_INV_PARAMS;
                }
            case SEVEN:
                ret = hexstr_to_unit(tok->arg[SIX], (u32 *)&cmd->lptstr.greenfield);
                if (ret) {
                    return -CMD_ERR_INV_PARAMS;
                }
            case SIX:
                ret = hexstr_to_unit(tok->arg[FIVE], (u32 *)&cmd->lptstr.rifs);
                if (ret) {
                    return -CMD_ERR_INV_PARAMS;
                }
            break;
        ...
        if (tok->arg_found == FOUR) {
            ...
            ret = string_to_uint(tok->arg[THREE], (u32 *)&cmd->ping.start);
            if (ret) {
                return -CMD_ERR_INV_PARAMS;
                cmd->ping.ext = 1;
...
int at_format_func(char *at_name, u8 set_opt, u8 update_flash, union HOSTIF_CMDRSP_PARAMS_UNION *cmdrsp,
    ...
    if (strcmp("QMAC", at_name) == 0 || strcmp("&HWV", at_name) == 0
        ...
        *res_len = sprintf(res_resp, "+OK=%02x%02x%02x%02x%02x%02x", 
        ...
        || (strcmp("BLETPS", at_name) == 0) || (strcmp("BLESSCM", at_name) == 0) || (strcmp("BLESND", at_name) == 0)
        || (strcmp("BTTEST", at_name) == 0)|| (strcmp("BLEDS", at_name) == 0)|| (strcmp("BLEDC", at_name) == 0)
        || (strcmp("BLEUM", at_name) == 0) || (strcmp("BLEADV", at_name) == 0) || (strcmp("BLEADATA", at_name) == 0)
        || (strcmp("BLESCRSP", at_name) == 0) || (strcmp("BLESCPRM", at_name) == 0)
        || (strcmp("BLESFLT", at_name) == 0) || (strcmp("BLESCAN", at_name) == 0)
        || (strcmp("BTAVS", at_name) == 0)|| (strcmp("BTHFP", at_name) == 0)
        || (strcmp("BTSPPS", at_name) == 0)|| (strcmp("BTSPPC", at_name) == 0)
        || (strcmp("BTSCM", at_name) == 0) || (strcmp("BLEDMADV", at_name) == 0)
        || (strcmp("BLEDMSCAN", at_name) == 0) || (strcmp("BTRF", at_name) == 0)
        ||(strcmp("BTINQUIRY", at_name) == 0)|| (strcmp("BLEDCMC", at_name) == 0)
        || (strcmp("BLEHIDS", at_name) == 0)|| (strcmp("BLEHIDU", at_name) == 0)
#endif
        ) {
        *res_len = atcmd_ok_resp(res_resp);
    } else if (strcmp("WJOIN", at_name) == 0) {
        ...
        *res_len = 0;
        ...
        if (cmdrsp->lkstt.status == 0) {
            ...
            *res_len = sprintf(res_resp, "+OK=%d,\"%d.%d.%d.%d\",\"%d.%d.%d.%d\",\"%d.%d.%d.%d\",\"%d.%d.%d.%d\",\"%d.%d.%d.%d\"",
                cmdrsp->lkstt.status,
                cmdrsp->lkstt.ip[0], cmdrsp->lkstt.ip[1], cmdrsp->lkstt.ip[TWO], cmdrsp->lkstt.ip[THREE],
                cmdrsp->lkstt.nm[0], cmdrsp->lkstt.nm[1], cmdrsp->lkstt.nm[TWO], cmdrsp->lkstt.nm[THREE],
                cmdrsp->lkstt.gw[0], cmdrsp->lkstt.gw[1], cmdrsp->lkstt.gw[TWO], cmdrsp->lkstt.gw[THREE],
                cmdrsp->lkstt.dns1[0], cmdrsp->lkstt.dns1[1], cmdrsp->lkstt.dns1[TWO], cmdrsp->lkstt.dns1[THREE],
                cmdrsp->lkstt.dns2[0], cmdrsp->lkstt.dns2[1], cmdrsp->lkstt.dns2[TWO], cmdrsp->lkstt.dns2[THREE]);
        }
            ...
            *res_len = atcmd_ok_resp(res_resp);
            ...
            if (cmdrsp->bssid.enable) {
                *res_len = sprintf(res_resp, "+OK=%u,%02x%02x%02x%02x%02x%02x", 
                        ...
                        cmdrsp->bssid.bssid[THREE], cmdrsp->bssid.bssid[FOUR], cmdrsp->bssid.bssid[FIVE]);
            } else {
        ...
        if (!set_opt) {
            if (cmdrsp->cntparam_bssid_en.bssid_enable) {
                *res_len = sprintf(res_resp, "+OK=%u,%02x%02x%02x%02x%02x%02x,",
                    ...
                    cmdrsp->cntparam_bssid_en.bssid[0], cmdrsp->cntparam_bssid_en.bssid[1],
                    cmdrsp->cntparam_bssid_en.bssid[TWO], cmdrsp->cntparam_bssid_en.bssid[THREE],
                    cmdrsp->cntparam_bssid_en.bssid[FOUR], cmdrsp->cntparam_bssid_en.bssid[FIVE]);
        ...
        *res_len = atcmd_ok_resp(res_resp);
        ...
        if (set_opt) {
            ...
            *res_len = sprintf(res_resp, "+OK=%04x,%04x,%hhu,%hhu,%hhu,%hhu,%hhu,%02X%02X%02X%02X%02X%02X",
            ...
            cmdrsp->bleprm.peer_addr[THREE], cmdrsp->bleprm.peer_addr[FOUR], cmdrsp->bleprm.peer_addr[FIVE]);
        }
#endif
    ...
    ||(strcmp("APNIP", at_name) == 0)
        ...
        if (set_opt) {
            ...
            *res_len = sprintf(res_resp, 
                        ...
                        cmdrsp->nip.ip[TWO], cmdrsp->nip.ip[THREE],
                        cmdrsp->nip.nm[0], cmdrsp->nip.nm[1],
                        cmdrsp->nip.nm[TWO], cmdrsp->nip.nm[THREE],
                        cmdrsp->nip.gw[0], cmdrsp->nip.gw[1],
                        cmdrsp->nip.gw[TWO], cmdrsp->nip.gw[THREE],
                        cmdrsp->nip.dns[0], cmdrsp->nip.dns[1],
                        cmdrsp->nip.dns[TWO], cmdrsp->nip.dns[THREE]);
        }
    } else if (strcmp("ATRM", at_name) == 0) {
        if (set_opt) {
            *res_len = atcmd_ok_resp(res_resp);
            ...
            *res_len = sprintf(res_resp, "+OK=");
            ...
            for (i = 0; i < cmdrsp->skstt.number; i++) {
                ...
                *res_len += sprintf(res_resp + (*res_len),
                    ...
                    ext->host_ipaddr[TWO], ext->host_ipaddr[THREE],
        ...
        *res_len = sprintf(res_resp, "+OK=%c%x.%02x.%02x.%02x%02x,%c%x.%02x.%02x@ %s %s",
                ...
                cmdrsp->ver.hw_ver[THREE], cmdrsp->ver.hw_ver[FOUR], cmdrsp->ver.hw_ver[FIVE],
                cmdrsp->ver.fw_ver[0], cmdrsp->ver.fw_ver[1], cmdrsp->ver.fw_ver[TWO],
                cmdrsp->ver.fw_ver[THREE], SysCreatedTime, SysCreatedDate);
        ...
        if (set_opt) {
            ...
            *res_len = sprintf(res_resp, "+OK=%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x"
            ...
            "%02x%02x", \
                ...
                cmdrsp->txg.tx_gain[THREE], cmdrsp->txg.tx_gain[FOUR], cmdrsp->txg.tx_gain[FIVE], \
                cmdrsp->txg.tx_gain[SIX], cmdrsp->txg.tx_gain[SEVEN], cmdrsp->txg.tx_gain[EIGHT], \
                cmdrsp->txg.tx_gain[NINE], cmdrsp->txg.tx_gain[TEN], cmdrsp->txg.tx_gain[TEN + 1],\
                cmdrsp->txg.tx_gain[TEN + TWO], cmdrsp->txg.tx_gain[TEN + THREE], cmdrsp->txg.tx_gain[TEN + FOUR],\
                cmdrsp->txg.tx_gain[TEN + FIVE], cmdrsp->txg.tx_gain[TEN + SIX], cmdrsp->txg.tx_gain[TEN + SEVEN],\
                cmdrsp->txg.tx_gain[TEN + EIGHT], cmdrsp->txg.tx_gain[TEN + NINE], cmdrsp->txg.tx_gain[TWENTY],\
                cmdrsp->txg.tx_gain[TWENTY + 1], cmdrsp->txg.tx_gain[TWENTY + TWO], cmdrsp->txg.tx_gain[TWENTY + THREE],\
                cmdrsp->txg.tx_gain[TWENTY + FOUR], cmdrsp->txg.tx_gain[TWENTY + FIVE], cmdrsp->txg.tx_gain[TWENTY + SIX],\
                cmdrsp->txg.tx_gain[TWENTY + SEVEN], cmdrsp->txg.tx_gain[TWENTY + EIGHT],
                cmdrsp->txg.tx_gain[TWENTY + NINE], cmdrsp->txg.tx_gain[THIRTY], cmdrsp->txg.tx_gain[THIRTY + 1], \
                cmdrsp->txg.tx_gain[THIRTY + TWO], cmdrsp->txg.tx_gain[THIRTY + THREE], cmdrsp->txg.tx_gain[THIRTY + FOUR],\
                cmdrsp->txg.tx_gain[THIRTY + FIVE], cmdrsp->txg.tx_gain[THIRTY + SIX], cmdrsp->txg.tx_gain[THIRTY + SEVEN],\
                cmdrsp->txg.tx_gain[THIRTY + EIGHT], cmdrsp->txg.tx_gain[THIRTY + NINE],\
                cmdrsp->txg.tx_gain[FORTY], cmdrsp->txg.tx_gain[FORTY + 1],\
                cmdrsp->txg.tx_gain[FORTY + TWO], cmdrsp->txg.tx_gain[FORTY + THREE], cmdrsp->txg.tx_gain[FORTY + FOUR],\
                cmdrsp->txg.tx_gain[FORTY + FIVE], cmdrsp->txg.tx_gain[FORTY + SIX], cmdrsp->txg.tx_gain[FORTY + SEVEN],\
                cmdrsp->txg.tx_gain[FORTY + EIGHT], cmdrsp->txg.tx_gain[FORTY + NINE], cmdrsp->txg.tx_gain[FIFTY],\
                cmdrsp->txg.tx_gain[FIFTY + 1], cmdrsp->txg.tx_gain[FIFTY + TWO], cmdrsp->txg.tx_gain[FIFTY + THREE],\
                cmdrsp->txg.tx_gain[FIFTY + FOUR], cmdrsp->txg.tx_gain[FIFTY + FIVE], cmdrsp->txg.tx_gain[FIFTY + SIX],\
                cmdrsp->txg.tx_gain[FIFTY + SEVEN], cmdrsp->txg.tx_gain[FIFTY + EIGHT],
                cmdrsp->txg.tx_gain[FIFTY + NINE], cmdrsp->txg.tx_gain[SIXTY],
                cmdrsp->txg.tx_gain[SIXTY + 1], cmdrsp->txg.tx_gain[SIXTY + TWO], \
                cmdrsp->txg.tx_gain[SIXTY + THREE], cmdrsp->txg.tx_gain[SIXTY + FOUR], cmdrsp->txg.tx_gain[SIXTY + FIVE], \
                cmdrsp->txg.tx_gain[SIXTY + SIX], cmdrsp->txg.tx_gain[SIXTY + SEVEN], cmdrsp->txg.tx_gain[SIXTY + EIGHT], \
                cmdrsp->txg.tx_gain[SIXTY + NINE], cmdrsp->txg.tx_gain[SEVENTY], cmdrsp->txg.tx_gain[SEVENTY + 1],\
                cmdrsp->txg.tx_gain[SEVENTY + TWO], cmdrsp->txg.tx_gain[SEVENTY + THREE], cmdrsp->txg.tx_gain[SEVENTY + FOUR],\
                cmdrsp->txg.tx_gain[SEVENTY + FIVE], cmdrsp->txg.tx_gain[SEVENTY + SIX], cmdrsp->txg.tx_gain[SEVENTY + SEVEN],\
                cmdrsp->txg.tx_gain[SEVENTY + EIGHT], cmdrsp->txg.tx_gain[SEVENTY + NINE], cmdrsp->txg.tx_gain[EIGHTY],\
                cmdrsp->txg.tx_gain[EIGHTY + 1], cmdrsp->txg.tx_gain[EIGHTY + TWO], cmdrsp->txg.tx_gain[EIGHTY + THREE]);
        }
    } else if (strcmp("&TXGI", at_name) == 0) {
        if (set_opt) {
            *res_len = atcmd_ok_resp(res_resp);
            ...
            *res_len = sprintf(res_resp, "+OK=%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x"
                ...
                cmdrsp->txg.tx_gain[TWENTY + 1], cmdrsp->txg.tx_gain[TWENTY + TWO], cmdrsp->txg.tx_gain[TWENTY + THREE],\
                cmdrsp->txg.tx_gain[TWENTY + FOUR], cmdrsp->txg.tx_gain[TWENTY + FIVE], cmdrsp->txg.tx_gain[TWENTY + SIX],\
                cmdrsp->txg.tx_gain[TWENTY + SEVEN]);
        }
    }
    ...
    if (ri_cmd_id == HOSTIF_CMD_REGR || ri_cmd_id == HOSTIF_CMD_FLSR) {
        ...
        do {
            ...
            if(len > socket.host_len) {
                if (socket.client == 1 && socket.host_len == FOUR) {
                    u32 ipaddr = get_unaligned_le32(p);
                    MEMCPY(socket.ip_addr, &ipaddr, FOUR);
        ...
        cmd->skghbn.ipstr[length - FOUR] = '\0';
#endif
        ...
        if (cmd->httpc.data_len > FIVE_HUNDRED_AND_TWELVE) {
            return CMD_ERR_INV_PARAMS;
        }
        ...
        cmd->httpc.url = p + EIGHT;
        cmd->httpc.data = p + EIGHT + cmd->httpc.url_len + 1;
        memcpy(p + EIGHT, url, cmd->httpc.url_len);
        p[cmd->httpc.url_len + EIGHT] = '\0';
        memcpy(p + NINE + cmd->httpc.url_len, url + cmd->httpc.url_len, cmd->httpc.data_len);
        p[NINE + cmd->httpc.url_len + cmd->httpc.data_len] = '\0';
        tls_mem_free(url);
        url = NULL;
    }
#endif
