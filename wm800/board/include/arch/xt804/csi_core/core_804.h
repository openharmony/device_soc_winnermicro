...
__STATIC_INLINE void csi_vic_set_threshold(uint32_t VectThreshold, uint32_t PrioThreshold)
{
    ...
    if (VectThresholdValue <= 31) { // 31:byte alignment
        VIC->IPTR = 0x80000000 | (((VectThresholdValue + 32) & 0xFF) << 8) | // 32:byte alignment, 8:byte alignment
            ((PrioThreshold & 0x3) << 6); // 6:byte alignment
    }

    if (VectThresholdValue > 31 && VectThresholdValue < 96) { // 31:byte alignment, 96:byte alignment
        VIC->IPTR = 0x80000000 | (((VectThresholdValue + 32) & 0xFF) << 8) | // 32:byte alignment, 8:byte alignment
            ((PrioThreshold & 0x7) << 5); // 5:byte alignment
    }

    if (VectThresholdValue > 95) { // 95:byte alignment
        VIC->IPTR = 0x80000000 | (((VectThresholdValue + 32) & 0xFF) << 8) | // 32:byte alignment, 8:byte alignment
            ((PrioThreshold & 0xF) << 4); // 4:byte alignment
    }
}
...
__STATIC_INLINE void csi_dcache_enable (void)
{
    ...
    CACHE->CIR = CACHE_CIR_INV_ALL_Msk;         /* invalidate all Cache */
    CACHE->CER =  (uint32_t)(CACHE_CER_EN_Msk | CACHE_CER_WB_Msk | CACHE_CER_DCW_Msk) & (~CACHE_CER_CFIG_Msk);
#endif
}
...
/**
  ...
  */
__STATIC_INLINE void csi_cache_set_range (uint32_t index, uint32_t baseAddr, uint32_t se, \
