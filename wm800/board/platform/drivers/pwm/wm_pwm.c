...
int tls_pwm_out_mode_config(u8 channel, enum tls_pwm_out_mode mode)
{
    ...
    if (WM_PWM_OUT_MODE_BRAKE == mode)
        ...
        /* enable the all synchronous mode mode */
        tls_reg_write32(HR_PWM_CTL,  tls_reg_read32(HR_PWM_CTL) | BIT(6));
    } else if (WM_PWM_OUT_MODE_2SYC == mode) {
        if (channel != 0 && channel != 2) // 2:byte alignment
            return WM_FAILED;
        /* disable the brake mode */
        tls_reg_write32(HR_PWM_BRKCTL, tls_reg_read32(HR_PWM_BRKCTL) & ~(0x1800<<channel));
        /* disable the all synchronous mode mode */
        tls_reg_write32(HR_PWM_CTL,  tls_reg_read32(HR_PWM_CTL) & ~BIT(6));
        /* enable the two channel synchronous mode */
        ...
        /* disable the all synchronous mode mode */
        tls_reg_write32(HR_PWM_CTL,  tls_reg_read32(HR_PWM_CTL) & ~BIT(6));
        /* disable the two channel synchronous mode */
        tls_reg_write32(HR_PWM_CTL,  tls_reg_read32(HR_PWM_CTL) & ~BIT(14 + channel / 2));
        /* enable the complementary mode */
        tls_reg_write32(HR_PWM_CTL,  tls_reg_read32(HR_PWM_CTL) | BIT(0 + channel / 2));
    } else if (WM_PWM_OUT_MODE_INDPT == mode) {
        tls_reg_write32(HR_PWM_CTL,    tls_reg_read32(HR_PWM_CTL)    & (~BIT(6)));
        if (channel != 4) { // 4:byte alignment
            tls_reg_write32(HR_PWM_CTL,    tls_reg_read32(HR_PWM_CTL)    & (~BIT(0 + channel / 2)));
            tls_reg_write32(HR_PWM_CTL,    tls_reg_read32(HR_PWM_CTL)    & (~BIT(14 + channel / 2)));
        }
        /* enable the independent mode */
